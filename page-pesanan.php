<?php
    ...
    $sql = "SELECT product.kodeProduk AS productCode, tbl_order.transaksiID, orderdetails.orderItemID, user.kodeUser AS PenjualID, user.username AS sellerName, tbl_order.kodeUser, 
            product.namaProduk AS namaProduk, product.gambar AS gambar, tbl_order.tgl_beli AS tanggal,provinsi.namaProvinsi AS lokasi, orderdetails.detailsOngkir AS ongkir,
            ...
            WHERE tbl_order.kodeUser = $userID AND orderdetails.status = 'proses' ORDER BY orderdetails.orderItemID DESC";
    ...
    $sql = "SELECT product.kodeProduk AS productCode,tbl_order.transaksiID, orderdetails.orderItemID ,user.kodeUser AS PenjualID, user.username AS sellerName, tbl_order.kodeUser, 
            ...
            (orderdetails.qty*orderdetails.harga) AS harga FROM tbl_order INNER JOIN orderdetails USING(transaksiID) INNER JOIN detailsproduct 
            USING(kodeItem) INNER JOIN product USING(kodeProduk) INNER JOIN user ON product.kodeUser = user.kodeUser INNER JOIN provinsi USING(idProvinsi) 
            WHERE tbl_order.kodeUser = $userID AND orderdetails.status = 'Sedang dikirim' ORDER BY orderdetails.orderItemID DESC";
    ...
    $sql = "SELECT product.kodeProduk AS productCode,tbl_order.transaksiID, orderdetails.orderItemID, user.kodeUser AS PenjualID, user.username AS sellerName, tbl_order.kodeUser, 
            ...
            (orderdetails.qty*orderdetails.harga) AS harga FROM tbl_order INNER JOIN orderdetails USING(transaksiID) INNER JOIN detailsproduct 
            USING(kodeItem) INNER JOIN product USING(kodeProduk) INNER JOIN user ON product.kodeUser = user.kodeUser INNER JOIN provinsi USING(idProvinsi) 
            WHERE tbl_order.kodeUser = $userID AND orderdetails.status = 'selesai' ORDER BY orderdetails.orderItemID DESC";
                ...
                <div class="col-9">
                    ...
                    <div class="container p-2">
                        ...
                        <div class="cart-items">
                            ...
                            <small class="mt-3 ml-3">Alamat:<br> <?=$alamat['alamat']?></small>
                            </div>
                            <?php } ?>
                            <br>
                        ...
                        <?php
                            ...
                            <div class="cart-item row rounded shadow p-4">
                                ...
                                <div class="col-10">
                                    <small class="float-right font-italic"><?=$row['tanggal']?></small>
                                    <br>
                                    <small class="float-right btn-primary btn btn-sm ml-1">Sedang dikirim</small>
                                    <a href="page-ulasan.php?id=<?=$row['orderItemID']?>" class="float-right btn btn-success btn-sm ml-1">Selesai</a>
                                    <p id="item-name"><?=$row['namaProduk'];?></p>
                                    <small class="float-right">Ongkos kirim: <span style="color:#0275d8; font-weight:bold;"><?=number_format($row['ongkir'],0,',','.');?></span></small>
                                    <br>
                                    <small class="float-right">Harga satuan: <span style="color:#0275d8; font-weight:bold;"><?=number_format($row['hargasatuan'],0,',','.');?> x <?=$row['qty'];?></span></small>
