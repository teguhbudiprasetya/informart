...
session_start();
    ...
    $sql = "SELECT cart.kodeUser AS PembeliID, cart.detailsOngkir AS ongkir, user.username AS sellerName ,product.kodeUser AS PenjualID, product.kodeProduk AS productCode ,product.namaProduk AS namaProduk, product.gambar AS gambar, provinsi.namaProvinsi AS lokasi, cart.id AS ID ,cart.qty AS qty, cart.warna AS warna, cart.ukuran AS ukuran
    FROM cart JOIN detailsproduct USING(kodeItem) JOIN product USING(kodeProduk) JOIN user ON product.kodeUser = user.kodeUser JOIN provinsi ON user.idProvinsi = provinsi.idProvinsi WHERE cart.kodeUser = $userID";
    ...
    if(isset($_POST['bayar'])){
        ...
        if($saldo >= $totalPriceFix){
            ...
            if(isset($potongan)){
                $sql = "INSERT INTO tbl_order VALUES ('', $userID, 'proses', '$now', '', $totalPriceFix, $ongkir, $potongan )";
            }else{
                $sql = "INSERT INTO tbl_order VALUES ('', $userID, 'proses', '$now', '', $totalPriceFix, $ongkir, 0 )";
            }
            ...
            if($insertOrder){
                $sql = "SELECT cart.*, product.harga, product.kodeProduk AS kodeProduk, user.kodeUser AS kodepenjual FROM cart INNER JOIN detailsproduct
                ...
                foreach($inputdetails as $row){
                    $inserdetails = "INSERT INTO orderdetails VALUES ('', $row[kodeItem], $row[qty], $row[harga], $next, $row[detailsOngkir], $row[idEkspedisi], 'proses',$row[kodepenjual])";
                    $sql = mysqli_query($conn, $inserdetails);
                    ...
                    $ratingPenjual = "UPDATE user SET penjualan = penjualan + 1 WHERE kodeUser = $row[kodepenjual]";
                    mysqli_query($conn, $ratingPenjual)or trigger_error("Query Failed! SQL: $ratingPenjual - Error: ".mysqli_error($conn), E_USER_ERROR);
                    
                    $productSold = "UPDATE product SET terjual = terjual + 1 WHERE kodeProduk = $row[kodeProduk]";
                    ...
                    <div class="cart-items">
                        ...
                        <div class="cart-item row">
                            ...
                            <div class="col-10">
                                ...
                                <small class="float-right">Qty: <?=$row['qty'];?> | Warna: <?=$row['warna'];?> | Ukuran: <?=$row['ukuran'];?> | <a href="delete-cart.php?id=<?=$row['ID']?>"><i class="fa fa-trash" aria-hidden="true" ></i></a></small>
